...
def run_review_mode() -> None:
    ...
    for idx, item in enumerate(items, 1):
        ...
        print(f"      Matched: {item['yt_title']} (Score: {item['score']})")
    ...
    print(" 1. Accept current match and download")
    ...
    if action == "1":
        # Force download using existing URL
        override_track = {
        ...
        if custom_url:
            ...
            # Modify best candidate directly
            status, result = download_single_spotify_track(override_track, int(selected["index"]))
            if status == "success":
                progress[key] = {
                    "title": selected["title"],
