...
try:
    ...
    from mutagen.id3 import ID3, TIT2, TPE1, TPE2, TALB, TRCK, USLT, APIC, ID3NoHeaderError
...
def apply_native_metadata(
    ...
    """
    Applies track tags (Title, Artist, Album Artist, Album, Cover Art, Lyrics, Track Number) directly
    to audio files using Mutagen without lossy re-encoding.
    ...
    try:
        if ext in [".m4a", ".mp4"]:
            ...
            if image_bytes:
                ...
                tags.add(APIC(encoding=3, mime=mime, type=3, desc="Cover (front)", data=image_bytes))
...
def crop_square_artwork(image_path: Path) -> Tuple[bool, str]:
    """Crops artwork (.webp / .jpg / .png) to a 1:1 square aspect ratio using FFmpeg."""
